...
client.on('ready', () => {
    ...
    client.setProfile = sql.prepare("INSERT OR REPLACE INTO profile (id, username, wood, stone, metal, gold, balance, experience) VALUES (@id, @username, @wood, @stone, @metal, @gold, @balance, @experience);");
});
...
client.on('message', message => {
    ...
    if (msg === prefix + 'profile') {
        ...
        if (!profile) {
            profile = { id: null, username: user.username, wood: 0, stone: 0, metal: 0, gold: 0, balance: 0, experience: 0 }
    ...
    if (msg === prefix + 'inv') {
        let user = message.mentions.users.first() || message.author;
         profile = client.getProfile.get(user.username);
        if (!profile) {
            profile = { id: null, username: user.username, wood: 0, stone: 0, metal: 0, gold: 0, experience: 0 }
            client.setProfile.run(profile);
        }
        message.channel.send({
            embed: {
    ...
    if (msg === prefix + 'break') {
        ...
        profile = client.getProfile.get(user.username);
         if (!profile) {
            profile = { id: null, username: user.username, wood: 0, stone: 0, metal: 0, gold: 0, balance: 0, experience: 1/10000 }
            client.setProfile.run(profile);
        }
